 ...
 * Provides vertex shader for Mixed Reality Design Language backplate rendering
 */

/**
 * Shader uniform parameters for backplate rendering
 */
export interface MRDLBackplateUniforms {
  ...
  /** Base corner radius for all corners */
  _Radius_: number;
  /** Width of the border line */
  _Line_Width_: number;
  /** Whether sizes are in absolute units or relative */
  ...
  /** Tint color for iridescence effect */
  _Iridescence_Tint_: [number, number, number, number];
  /** Iridescent map texture sampler */
  _Iridescent_Map_: WebGLTexture;
  
  /** Angle for directional effects (in degrees) */
  _Angle_: number;
  /** Whether iridescence is reflected */
  _Reflected_: boolean;
  /** Frequency for wave/gradient effects */
  _Frequency_: number;
  /** Vertical offset for gradient */
  ...
  /** Color at top-right corner */
  _Top_Right_: [number, number, number, number];
  /** Color at bottom-left corner */
  _Bottom_Left_: [number, number, number, number];
  /** Color at bottom-right corner */
  _Bottom_Right_: [number, number, number, number];
  
  /** Width of edge highlight */
  _Edge_Width_: number;
  /** Power factor for edge falloff */
  _Edge_Power_: number;
 ...
 */
export interface MRDLBackplateAttributes {
 ...
 */
export interface MRDLBackplateVaryings {
  ...
  vUV: [number, number];
  /** Tangent vector (used for highlight animation) */
  vTangent: [number, number, number];
  /** Binormal vector (directional vector) */
  vBinormal: [number, number, number];
  /** Extra parameters 1: rectangle parameters (xy: half-size, z: gradient1, w: gradient2) */
  vExtra1: [number, number, number, number];
  /** Extra parameters 2: color UV info and size info (xy: color UV, zw: radius and line width) */
  vExtra2: [number, number, number, number];
}

/**
 * MRDL Backplate vertex shader source code
 */
export const MRDL_BACKPLATE_VERTEX_SHADER: string = `
uniform mat4 world;
 ...
 */
export const MRDL_BACKPLATE_VERTEX_SHADER_NAME: string = "mrdlBackplateVertexShader";

/**
 * Complete shader definition object
 */
export interface MRDLBackplateShaderDefinition {
  /** Shader name identifier */
  name: string;
...
/**
 * Export the complete shader definition
 */
export const mrdlBackplateVertexShader: MRDLBackplateShaderDefinition;