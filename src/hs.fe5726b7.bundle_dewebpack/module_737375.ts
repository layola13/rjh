...
interface CSSLoaderResult extends CSSModule {
  toString(): string;
  i(modules: CSSModule[], mediaQuery?: string): void;
}

const cssContent: string = `.dropdownlist {
...
export function createCSSModule(cssLoader: (sourceMap: boolean) => CSSLoaderResult, moduleId: string): CSSLoaderResult {
  const module = cssLoader(false);
  module.push([moduleId, cssContent]);
  return module;
}