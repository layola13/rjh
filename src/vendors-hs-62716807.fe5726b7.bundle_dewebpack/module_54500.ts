...
/**
 * Returns the absolute value using bitwise operations
 */
...
/**
 * Returns the base-2 logarithm of a number (integer)
 */
export function log2(value: number): number {
    let result = (value > 65535 ? 1 : 0) << 4;
    let temp = ((value >>>= result) > 255 ? 1 : 0) << 3;
    result |= temp;
    temp = ((value >>>= temp) > 15 ? 1 : 0) << 2;
    result |= temp;
    temp = ((value >>>= temp) > 3 ? 1 : 0) << 1;
    result |= temp;
    return result | ((value >>>= temp) >> 1);
}

/**
 * Returns the base-10 logarithm of a number (integer)
 */
...
export function countTrailingZeros(value: number): number {
    ...
    if (value & 16711935) count -= 8;
    if (value & 252645135) count -= 4;
    if (value & 858993459) count -= 2;
    if (value & 1431655765) count -= 1;
    return count;
}

/**
 * Returns the next power of 2 greater than or equal to the value
 */
...
/**
 * Returns the previous power of 2 less than or equal to the value
 */
...
/**
 * Returns the parity of the number (1 if odd number of bits, 0 if even)
 */
...
(function initializeReverseTable(table: number[]): void {
    for (let i = 0; i < 256; ++i) {
        let original = i;
        let reversed = i;
        let shift = 7;
...
/**
 * Extracts one component from an interleaved 32-bit integer
 */
...
/**
 * Interleaves three 10-bit integers into a single 30-bit integer
 */
...
/**
 * Extracts one component from an interleaved 30-bit integer
 */
