...
export class Triangle {
  ...
  toString(): string {
    return `[${toString(this.points_[0])}${toString(this.points_[1])}${toString(this.points_[2])}]`;
  }
  ...
  setConstrainedEdgeCW(point: Point, isConstrained: boolean): void {
    if (point === this.points_[0]) {
      this.constrained_edge[1] = isConstrained;
    } else if (point === this.points_[1]) {
      this.constrained_edge[2] = isConstrained;
    } else {
      this.constrained_edge[0] = isConstrained;
    }
  }
  ...
  setDelaunayEdgeCW(point: Point, isDelaunay: boolean): void {
    if (point === this.points_[0]) {
      this.delaunay_edge[1] = isDelaunay;
    } else if (point === this.points_[1]) {
      this.delaunay_edge[2] = isDelaunay;
    } else {
      this.delaunay_edge[0] = isDelaunay;
    }
  }
  ...
  oppositePoint(triangle: Triangle, point: Point): Point | null {
    const clockwisePoint = triangle.pointCW(point);
    return clockwisePoint ? this.pointCW(clockwisePoint) : null;
  }
