...
const TEMPERATURE_KEYS = Object.keys(TEMPERATURE_COLOR_MAP).map((key) =>
  parseInt(key, 10)
);
TEMPERATURE_KEYS.push(Infinity);

const DEFAULT_TEMPERATURE_KEY = Object.keys(TEMPERATURE_COLOR_MAP)[0];

/**
 * Finds the nearest temperature color mapping for a given temperature value.
 * @param temperature - The temperature value (as string or number)
...
export function nearestTemperature(temperature: string | number): RGBAColor {
  try {
    const targetTemp = Number.parseInt(temperature.toString(), 10);
    const minTemp = TEMPERATURE_KEYS[0];
    let minDistance = targetTemp - minTemp;
    let nearestTemp = minTemp;

    for (let index = 0; index < TEMPERATURE_KEYS.length; index++) {
      const currentTemp = TEMPERATURE_KEYS[index];

      if (currentTemp <= targetTemp) {
        minDistance = targetTemp - currentTemp;
        continue;
      }

      if (currentTemp - targetTemp < minDistance) {
        nearestTemp = currentTemp;
        break;
      }

      nearestTemp = targetTemp - minDistance;
      break;
    }

    return TEMPERATURE_COLOR_MAP[nearestTemp];
  } catch (error) {
    return TEMPERATURE_COLOR_MAP[DEFAULT_TEMPERATURE_KEY];
  }
}