...
export class LightSetLibrary {
  ...
  static loadDefault(): void {
    ...
    Object.values(LightTypeEnum).forEach((lightType) => {
      this.add({
        key: lightType,
        label: lightType,
      });
  ...
  static get(key: string): LightSetData | undefined {
    return this._lightData.get(key);
  }

  static add(data: LightSetData): void {
    this._lightData.set(data.key, data);
  }

  static delete(key: string): void {
    this._lightData.delete(key);
  }
}