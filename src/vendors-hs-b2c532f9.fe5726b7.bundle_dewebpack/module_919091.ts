...
const zhTwLocale: LocaleSpecification = {
  ...
  meridiem: (hour: number, minute: number, isLower: boolean): string => {
    const timeInMinutes = 100 * hour + minute;
    ...
    if (timeInMinutes < 1130) {
      return "上午";
    }
    if (timeInMinutes < 1230) {
      return "中午";
    }
    if (timeInMinutes < 1800) {
      return "下午";
    }
    return "晚上";
  },
...
moment.defineLocale("zh-tw", zhTwLocale);
