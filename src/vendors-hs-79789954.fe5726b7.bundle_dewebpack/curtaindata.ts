...
export class CurtainData {
  ...
  /**
   * Finds the default curtain ID that best matches the given X size
   * @param xSize - The target X dimension to match
   * @returns The ID of the curtain with the closest X size
   */
  static getDefaultSeekIdByXSize(xSize: number): string {
    const curtainsWithDiff: CurtainWithDiff[] = this._defaultCurtains.map(
      (curtain): CurtainWithDiff => ({
        item: curtain,
        diff: Math.abs(curtain.size.x - xSize)
      })
    );

    const sortedByDiff = curtainsWithDiff.sort(
      (a, b) => a.diff - b.diff
    );

    return sortedByDiff[0].item.id;
  }
